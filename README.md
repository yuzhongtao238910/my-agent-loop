...
请帮我读取a.txt b.txt c.txt 三个文件的内容并且输出结果
如果读取b的时候失败，其他的还需要继续读取哈


## 提示词
- 使用子任务来查找本项目安装了哪些第三方模块
- 使用子任务来读取当前目录下的所有.py文件，不需要读取依赖包下面得py文件，并且总结每个文件的功能
- 使用子任务创建一个快速排序文件，然后需要主agent来验证这个文件写得是否正确


## 目前subagent是一个阻塞得过程
- 子代理和主代理之间得通信是如何进行：子代理得output结论会放到这个父agent得message里面


## claude code
- by the way
- /btw 核心机制，临时子任务的思路，但是细节不太一致
- /btw 不是开启独立的子进程，而是在当前的会话之中创建一个轻量级别的临时链条，复用主会话的上下文和提示词的缓存
- 但是回答后的结果，不会写入主对话的历史，实现零污染并行回答

## 加载skill
- skillloading 技能按需要注入 只在任务需要的时候注入
- 用的时候才会加载
- /commit 按照仓库规范来创建git提交
- /test 
- /deploy 将应用部署到这个目标环境
- 1）精简摘要metadata，所有的技能的摘要来注入这个system prompt
- 模型来识别技能，触发这个skill工具来进行完成注入
- 2）按需加载
- skill全部加载导致提示词，message context直接膨胀 在ai大赛之中很多人的token很快被清空
- 可以加载多个技能

## 双层架构
- 第一层：始终存在，比较小
- 第二层：按需加载，比较详细，优雅分离
- 如果直接装入到这个系统提示词的话，就会每次都会带着

## skill loading
- 启动的时候注入目录，运行的时候会按需加载内容
- load_skill 
- build_system 启动的时候扫描skill的目录，读取所有的技能，提取出来这个摘要
